...
def get_3d_contact_points(image_path, intrinsics, extrinsics, depth_image, vrb_model, args):
    ...
    img = img.resize((1008, 756))  # Model expects this size
    ...
    for (u, v) in contact_points_scaled.astype(int):
        ...
        point_tsdf = Rt4x4 @ point_camera
        contact_3d_points.append(point_tsdf[:3])
...
# Order 3D points by proximity to their centroid.
def sort_points_by_centroid_distance(points):
    """
    Parameters:
    points : array-like, shape (N, 3) Points in world coordinates.
    """

    pts = np.asarray(points, dtype=float)
    if pts.size == 0:
        return pts, np.array([]), np.zeros(3)
    print(f"[compute_heatmap_3d] input points: {pts.shape[0]}")

    centroid = pts.mean(axis=0) # Calculate centroid
    dists = np.linalg.norm(pts - centroid, axis=1) # Calculate distance to centroid for each point
    order = np.argsort(dists) # sort distances
    return pts[order], dists[order], centroid
